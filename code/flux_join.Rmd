...
romsi <- crossing(xi_rho=1:dim(romsdepths)[2],eta_rho=1:dim(romsdepths)[2]) %>% mutate(cellindex=row_number())
...
box_fluxes <- box_fluxes_L %>% 
  left_join(box_fluxes_R,by=c('box','atlantis_layer')) %>% 
  # net flow
  rowwise() %>% 
  mutate(net_change=sum(totflux_in_from_right,totflux_in_from_left)) %>% 
  # hmm, trying to figure out how to do this next step
  group_by(box) %>% 
  # net downwelling (positive is down), converted to Sv (1x10^6 m3/s)
  mutate(net_downwelling=cumsum(net_change)*1e-6)
```
