...
**TODO**: Come back to this and try and pull w with Mike Sumner's angstroms::romshcoords() but using Cs_w and s_w instead - not sure if it will work.

```{r}
ma <- function(x, n = 2){
  moving_average <- as.numeric(stats::filter(x, rep(1 / n, n), sides = 2))
...
interpolate_var <- function(variable){
  ...
  if(variable=='w'){
    interp_dat <- interp_dat %>% mutate(evar=purrr::map(evar,ma)) %>% # interpolate between w points, to have the same layers as h
      drop_na() # for some reason, some rho points do not have w values (scroll down dat to see some examples). In other ROMS this may happen to other variables too, so keep an eye on this one when porting it to other systems
    }
...
boxes_roms_salt_temp %>% 
  ...
  ggplot(aes(salt_interval,salt_interp,color=atlantis_layer))+
  geom_point()+
...
Remember, that here we assume that $u$ and $v$ have the same h as $\rho$ points.
```{r}
...
```
Hard to tell what is sensible here. 
