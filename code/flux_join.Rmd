...
boxes_rho_thin <- boxes_rho_join %>% 
  st_set_geometry(NULL) %>% 
  dplyr::select(.bx0,xi_rho,eta_rho,rhoidx) %>% 
  drop_na()
...
match_interpolated_data <- function(interp_dat,rhoidx_vec,minz,maxz){
  interp_var<- interp_dat %>% 
    ...
    # filter depths
    mutate(interp_within_depths=purrr::map(interp,function(df) df %>% filter(between(depth,maxz,minz)))) %>% 
...
boxes_salt_temperature_interp <- boxes_depths_rhopts %>% 
  # apply the function to the interpolated salinity data
