...
atlantis_z <- c(-50,-50,-100,-350,-650,-1200)
```

```{r,echo=F}
tibble(dz=c(0,atlantis_z,-1),lyr=8:1) %>% 
...
face_depths <- faces %>% 
  ...
  rowwise() %>%
  mutate(face_area=min(left_area, right_area)) # I think this should be the area of the smaller face instead of the average of the two? Imagine a box to have 6 layers, and its adjacent box only 2. The 4 layers closest to the bottom of the deep box do not communicate with the shallower adjacent box (because layers 3-6 do not exists in the shallow box). 
```
...
romsdepthsdf <- as_tibble(romsdepths,dim=F) %>% 
  arrange(cellindex) %>% 
...
fluxes <- test_uv_albi %>% 
  mutate(gross = new_vec * face_area) %>%
...
Upwelling and downwelling represent $w$. The input to HydroConstruct seems to need $w$ expressed as some variation of the following language (from SEAP and Hawaii, respectively):
...
So it sounds like producing the net flux should be the right thing to do, and the units are the same as our fluxes. Double-check this (e.g. that we do not actually need it by unit space for some reason).

Notably, HydroConstruct seems to consider upward fluxes as positive.
```{r}
box_fluxes <- box_fluxes_L %>% 
  full_join(box_fluxes_R,by=c('box','atlantis_layer')) %>% 
  # net flow
  rowwise() %>% 
  mutate(net_flux=sum(totflux_in_from_right,totflux_in_from_left)) %>% 
