...
And which faces do not have u and v points?
```{r}
...
match_interpolated_data <- function(interp_dat,rhoidx_vec,minz,maxz){
  interp_var<- interp_dat %>% 
    ...
    bind_rows() %>% 
    pluck('val') %>% 
    mean(na.rm=TRUE) # this produces NaN if all values are NA's (e.g. for maxz=0)
...
**UPDATE 11/1/2021** Regarding this, I fixed it so that for each rho point it takes the value of $w$ closest to the interface. If the interface is shallower than the rho point (i.e. normal case for most layers), the value of w at the depth break is taken; if the interface is deeper than the deepest vertical layer for that rho point, the next closest value is taken. This yields many more points in the bottom layers and will hopefully help with flux balancing too.

**IMPORTANT**: I am not able to see, in this ROMS input file, whether a negative w corresponds to an upward or downward velocity. This is crucial to this step and it may change between ROMS, so we need to find a way to automatically recognize it or check for it (HydroConstruct considers positive fluxes as going *up*, and negative fluxes as going *down*). Convention for ROMS, including the NEP 10K and CGOA 3K, is that negative $w$ indicates an upward water velocity, but one should always check in their ROMS or else it may all be upside down.
...
```
**NOTE**: this step above finds $u$ and $v$ points matches also in those areas of the faces that do not exist. For example, if a box is 2000 m deep and its neighbor is 350 m deep, the method above will match all the $u$ and $v$ points with the face between the two boxes all the way down to -2000 m. The following step of multiplying the velocities across a face by the area of the face to obtain the flux solves the problem, in that velocities deeper than 350 m (in this example) will be dropped.
**NOTE 2**: this last step returns a lot of NAs. These seem to be in the deep layers where no $u$ and/or $v$ points are deep enough for a layer of a box. That is, the deepest layer of a face may have fewer $u$ and/or $v$ points than its shallower counterparts, or none. In a way this is similar to the vertical velocities through the bottom layers of the deep boxes. However, while in that case we used the closest value of $w$, here it is not that simple because we are not pulling values of the velocity at an intreface but we are averaging over all points within the buffer around a face. In addition, we cannot pull values from shallower layers because those are already used by shallower layers of the face, as opposed to shallower values of $w$ that simply fall just a little above the interface. Default behavior is to keep the NAs and lack of flux where the model domain goes much deeper than the ROMS.
...
fluxes_out <- rbind(fluxes_out_left,fluxes_out_rigth) %>% # Polygon_number has the value of a focal box now
  distinct() %>% 
