...
If one follows the format of the PS file, layers are numbered from 0 to 6, where 0 is the deepest and 6 seems to be the sediment. This highlights that we need to review the numbering throughout.

```{r}
depth_layer <- atlantis_depths %>% select(.bx0,atlantis_layer,dz) %>%
  pivot_wider(test,id_cols=.bx0,names_from=atlantis_layer,values_from = dz) %>%
  mutate(sed=1)%>%
  set_names('box_id','layer0','layer1','layer2','layer3','layer4','layer5','layer6')

write.table(depth_layer,'../outputs/depth_layer.csv', quote=FALSE, row.names = FALSE, sep = ',')
```
