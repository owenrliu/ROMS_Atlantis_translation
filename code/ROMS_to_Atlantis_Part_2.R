...
file_statevars <- '../../outputs/complete_from_Emily/Script1/2020/state_vars_test.dat'
file_transport <- '../../outputs/complete_from_Emily/Script1/2020/transport_test.dat'

roms_state_vars <- read.csv(file_statevars,sep='\t',header = T)
roms_state_vars <- roms_state_vars %>% filter(Polygon.number >= 0 & Time.Step > 10) # this is a hack to deal with a glitch from 2020...
...
#############################################################
# Create directories
dir.create('../../outputs/complete_from_Emily/Script1/2020/monthly')
dir.create('../../outputs/complete_from_Emily/Script1/2020/monthly/pre-interp')
dir.create('../../outputs/complete_from_Emily/Script1/2020/monthly/post-interp')
...
lapply(state_var_files, function(x) {
  ...
  write.table(x1, file=paste0('../../outputs/complete_from_Emily/Script1/2020/monthly/pre-interp/',x$Monthyear[1],'_statevars.dat'), quote=FALSE, row.names = FALSE, sep='\t')})
...
lapply(hydro_files, function(x) {
  x1 <- x %>% select(-Month,-Year,-Monthyear)
  write.table(x1, file=paste0('../../outputs/complete_from_Emily/Script1/2020/monthly/pre-interp/',x$Monthyear[1],'_hydro.dat'), quote=FALSE, row.names = FALSE, sep='\t')})
...
roms_files <- list.files('../../outputs/complete_from_Emily/Script1/2020/monthly/pre-interp',full.names = TRUE)
...
roms_dat_file_cleaner <- function(roms_file){
  ...
  if(grepl('statevars',roms_file)){ # if state variables, fill empty boxes
    ...
    #############################################################
    # Second, for some years (2020), the depth layer column comes out as character...
    roms_data$Depth.Layer..m. <- as.numeric(roms_data$Depth.Layer..m.)
    roms_data <- roms_data %>% filter(!is.na(Depth.Layer..m.))
  ...
  if(grepl('statevars',roms_file)) {
    ...
    write.table(roms_data_interp, paste0('../../outputs/complete_from_Emily/Script1/2020/monthly/post-interp/',monthyear,'_avg.dat'),
    ...
    write.table(roms_data_interp, paste0('../../outputs/complete_from_Emily/Script1/2020/monthly/post-interp/',monthyear,'_flux.dat'),
                quote=FALSE, row.names = FALSE, sep = '\t')
