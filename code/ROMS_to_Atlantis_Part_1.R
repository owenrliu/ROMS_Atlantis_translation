#################################
# Step 1: from ROMS NetCDF output to DAT files with state variables per layer per box and fluxes per face
#################################
...
roms_to_atlantis <- function(this_file){
  this_roms <- tidync(this_file)
  
  this_roms_vars <- hyper_grids(this_roms) %>% # all available grids in the ROMS ncdf
    pluck("grid") %>% # for each grid, pull out all the variables associated with that grid and make a reference table
    purrr::map_df(function(x){
      roms %>% activate(x) %>% hyper_vars() %>% 
        mutate(grd=x)
  ...
  # extract time
  time_grd <- this_roms_vars %>% filter(name=="ocean_time") %>% pluck('grd')
  roms_time <- this_roms %>% activate(time_grd) %>% hyper_tibble() %>% pull()
  ...
  interpolate_var <- function(variable, time_step){
    grd <- this_roms_vars %>% filter(name==variable) %>% pluck('grd')
    # pull the env data
  ...
  purrr::map(roms_time, purrr::possibly(interpolate_ts,NA)) # TODO: think about the ramifications for missing time steps
  
}

# apply to all ROMS files

purrr::map(roms_files_list,purrr::possibly(roms_to_atlantis,NA))
...
atlantis_depths_rev <- atlantis_depths %>%
  group_by(.bx0) %>%
  mutate(island=ifelse(sum(dz)>0,0,1)) %>% # adding a flag for island boxes to use for filtering below
  ungroup() %>%
  filter(dz>0|island==1) %>%
  group_by(.bx0) %>%
...
# turn zero-depths of island boxes to NA's, so that they can be written out as '_'
